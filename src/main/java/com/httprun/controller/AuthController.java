...
public class AuthController {
    ...
    public ResponseEntity<Token> createToken(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Token 创建参数：name(名称), subject(授权范围), isAdmin(是否管理员), expiresIn(过期时间/小时)", content = @Content(schema = @Schema(example = "{\"name\":\"my-token\",\"subject\":\"*\",\"isAdmin\":false,\"expiresIn\":24}"))) @RequestBody Map<String, Object> request) {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "验证结果返回", content = @Content(schema = @Schema(example = "{\"valid\":true,\"name\":\"my-token\",\"subject\":\"*\",\"isAdmin\":false}"))),
            @ApiResponse(responseCode = "400", description = "Token 格式错误")
    })
    public ResponseEntity<Map<String, Object>> validateToken(
            @Parameter(description = "JWT Token", required = false) @RequestHeader(value = "x-token", required = false) String token) {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Token 刷新成功", content = @Content(schema = @Schema(example = "{\"token\":\"new-jwt-token-here\"}"))),
            @ApiResponse(responseCode = "400", description = "Token 无效或已过期")
    })
    public ResponseEntity<Map<String, Object>> refreshToken(
            @Parameter(description = "JWT Token", required = true) @RequestHeader("x-token") String token) {
