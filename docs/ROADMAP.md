...
| 🟢 低 | 操作二次确认 | 高危命令执行前二次确认（前端） | 1天 | ⏳ 待开发 |

### 2. 执行体验优化

| 优先级 | 功能 | 说明 | 预估工时 | 状态 |
|--------|------|------|----------|------|
| 🔴 高 | 实时输出 | WebSocket 实时展示命令输出流 | 3天 | ⏳ 待开发 |
| 🔴 高 | 命令执行历史 | 展示历史执行记录，支持筛选和查看详情 | 2天 | ⏳ 待开发 |
| 🟡 中 | 批量执行 | 支持选择多个命令顺序/并行执行 | 3天 | ⏳ 待开发 |
| 🟡 中 | 命令收藏 | 收藏常用命令，快速访问 | 1天 | ⏳ 待开发 |
| 🟡 中 | 参数预设 | 保存常用参数组合，快速填充 | 2天 | ⏳ 待开发 |
| 🟢 低 | 暗黑模式 | 前端主题切换 | 1天 | ⏳ 待开发 |
...
| 异步执行优化 | 长时间命令占用连接 | 🟡 中 | WebSocket 实时输出 |
