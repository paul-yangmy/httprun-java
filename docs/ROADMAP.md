...
| 优先级 | 功能 | 说明 | 预估工时 | 状态 |
|--------|------|------|----------|------|
| 🔴 高 | SSH 连接池 | 复用 SSH Session，减少连接建立开销（参考 Apache Commons Pool） | 3天 | ⏳ 待开发 |
| 🔴 高 | 连接健康检查 | 连接池中 Session 定期心跳检测，自动剔除失效连接 | 1天 | ⏳ 待开发 |
| 🟡 中 | SSH 指纹管理 | 首次连接记录主机指纹，后续连接自动验证（防中间人攻击） | 2天 | ⏳ 待开发 |
| 🟡 中 | 连接超时优化 | 区分连接超时 / 执行超时，支持独立配置 | 1天 | ⏳ 待开发 |
...
| SSH 连接每次新建 | 频繁连接同一主机开销大 | 🔴 高 | 引入 SSH 连接池（Commons Pool2） | ⏳ 待开发 |
